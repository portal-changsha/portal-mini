<template>
  <div class="myreport app">
    ...
    <div class="clinic-info" v-for="(item,index) in showPrescriptionList" :key="index" >
      ...
      <div class="content row row-center" @click="goDetail(item)">
        <div class="left">
          ...
          <p>开单时间  {{item.recipes[0].recipeTime | formatDate}}</p>
        </div>
        <div class="arrow">
    ...
    <no-record :condition="prescriptionLists.length === 0" :tips="'抱歉，没有处方记录!'"></no-record>
  </div>
</template>
<script>
import hosVoucherSelect from '@/components/hos-voucher-selcet/hos-voucher-selcet.vue'
import { getReceipe } from '@/services/clinic.js'
...
export default {
  ...
  computed:{
	  showPrescriptionList(){
		  return this.prescriptionLists.filter(item => item.recipeType!=='4')
	  }
  },
  onLoad () {
    let today = new Date()
  ...
  methods: {
    ...
    async initData () {
      ...
      let res = await getReceipe(params)
      if (res.resultCode === this.$consts.RESULT_SUCCESS) {
		if(!res.data) {
			this.prescriptionLists = []
			showModal('提示', '当前医院暂未开放处方查询功能', false)
			return
		}
		this.prescriptionLists = res.data.visitInfos
      }
    },
    goDetail (item) {
	  this.$store.dispatch('paramStore/updateParam', {
		  namespace: 'prescriptionDetail',
		  details: item
	  })
	  this.$Router.push({ name: 'prescriptionDetail' })
    },
    select (e) {
		this.$store.dispatch('paramStore/updateParam', {
			namespace: 'DistrictSelect',
			target: 'myPrescription',
		})
