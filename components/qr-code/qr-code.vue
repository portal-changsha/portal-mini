	...
	export default {
		...
		methods: {
			...
			drawBackgroundImage(options) {
				let ctx = uni.createCanvasContext(this.cid, this)
			...
			drawLogo(options) {
				...
				let logoSize = this.size / 4
				let logoX = this.size / 2 - logoSize / 2
				let logoY = logoX

				let borderSize = logoSize + 10
				let borderX = this.size / 2 - borderSize / 2
				let borderY = borderX
				let borderRadius = 5
